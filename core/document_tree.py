...
class DocumentTreeBuilder:
    ...
    def add_to_tree(self, breadcrumbs: List[str], title: str, file_path: Path) -> None:
        ...
        current.children[title] = TreeNode(
            name=title, path=file_path.relative_to(self.input_dir)
        )
    ...
    def generate_table_format(
        self, node: TreeNode, parent_path: List[str] = None
    ) -> List[List[str]]:
        ...
        sorted_children = sorted(
            node.children.items(), key=lambda x: (not bool(x[1].children), x[0].lower())
        )
...
def main():
    ...
    parser.add_argument(
        "-i", "--input-dir", type=str, help="Input directory containing HTML files"
    )

    parser.add_argument(
        "-o", "--output-dir", type=str, help="Output directory for markdown file"
    )
    ...
    parser.add_argument(
        "--format", choices=["tree", "table"], help="Output format (tree or table)"
    )

    parser.add_argument(
        "--dry-run", action="store_true", help="Preview markdown output without saving"
    )
