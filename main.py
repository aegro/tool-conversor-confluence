...
class HTMLProcessorCLI:
    ...
    def parse_arguments(self) -> argparse.Namespace:
        ...
        parser.add_argument(
            "-o", "--output-dir", type=str, help="Output directory for processed files"
        )
        ...
        parser.add_argument(
            "--version", action="version", version="HTML Processor v1.0.0"
        )
    ...
    def print_summary(self) -> None:
        ...
        self.logger.info("-" * 40)
        self.logger.info(
            f"Total Input Files: {self.stats['files_processed'] + len(self.stats['errors'])}"
        )
        self.logger.info(
            f"Files Processed Successfully: {self.stats['files_processed']}"
        )
        self.logger.info(f"Files Failed: {len(self.stats['errors'])}")
        self.logger.info(f"DOCX Files Created: {self.stats['docx_created']}")
        self.logger.info(
            f"Files Not Processed: {self.stats.get('files_not_processed', 0)}"
        )
